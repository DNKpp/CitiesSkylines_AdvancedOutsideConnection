...
namespace AdvancedOutsideConnection
{
	...
	public static class Utils
	{
		...
		public static void ModMaterialIfNecessary(ref TransferManager.TransferReason material, int[] ratios, TransferManager.TransferReason[] expectedMaterials)
		{
			...
			if (index != -1)
			{
				var rnd = SimulationManager.instance.m_randomizer.Int32((uint)ratios.Sum());
				int newMatIndex = 0;
		...
		{
			return GetDefaultRatioArray(5000, ImportResources.Length);
		}

		public static int[] GetDefaultExportResourceRatio()
		{
			return GetDefaultRatioArray(5000, ExportResources.Length);
		}

		public static int[] GetDefaultRatioArray(int defaultValue, int length)
		{
			var ratios = new int[length];
			for (int i = 0; i < length; ++i)
			{
				ratios[i] = defaultValue;
			}
			return ratios;
		}
